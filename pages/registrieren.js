...
const LAENDER_DE = ["Deutschland","Österreich","Schweiz","Frankreich","Italien","Spanien","Portugal","Niederlande","Belgien","Dänemark","Schweden","Norwegen","Finnland","Polen","Tschechien","Slowakei","Ungarn","Kroatien","Griechenland","Irland","England","Schottland","Wales","Estland","Lettland","Litauen","Slowenien","Rumänien","Bulgarien","Serbien","Anderes"]
const LAENDER_EN = ["Germany","Austria","Switzerland","France","Italy","Spain","Portugal","Netherlands","Belgium","Denmark","Sweden","Norway","Finland","Poland","Czech Republic","Slovakia","Hungary","Croatia","Greece","Ireland","England","Scotland","Wales","Estonia","Latvia","Lithuania","Slovenia","Romania","Bulgaria","Serbia","Other"]
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch('https://formspree.io/f/xzdloqrg', {
        ...
        body: JSON.stringify({
          name: form.name, email: form.email, land: form.land, typ: form.typ, language: lang,
            ...
            <form onSubmit={handleSubmit} className={styles.form}>
              ...
              <div className={styles.fieldWrap}>
                ...
                <input type="text" required className={styles.input}
                  placeholder={form.typ==='kommune'?t('reg_name_placeholder_commune'):t('reg_name_placeholder_person')}
                  value={form.name} onChange={e=>update('name',e.target.value)}/>
              </div>
              <div className={styles.fieldWrap}>
                <label className={styles.label}>{t('reg_email')} *</label>
                <input type="email" required className={styles.input} placeholder="email@example.com"
              ...
              <input type="text" name="_gotcha" style={{display:'none'}} tabIndex="-1" autoComplete="off"
                value={form.honeypot} onChange={e=>update('honeypot',e.target.value)}/>
