...
// Ortssuche wie bei Google Maps: Freitext -> Koordinaten, über den offenen
// Adressdienst von OpenStreetMap (Nominatim), client-seitig aufgerufen.
export async function geocodeSearch(query) {
  const url = `https://nominatim.openstreetmap.org/search?format=jsonv2&q=${encodeURIComponent(query)}&limit=1&accept-language=de`
  const res = await fetch(url, { headers: { 'Accept': 'application/json' } })
  if (!res.ok) throw new Error(`Nominatim HTTP ${res.status}`)
  const data = await res.json()
